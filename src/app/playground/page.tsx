...
export default function PlaygroundPage() {
  const { user, loading, logout, login, authChecked } = useAuth();
  ...
  // Handle redirect when user is not authenticated after auth check
  useEffect(() => {
    // Only redirect if auth is checked AND we're sure there's no user
    // Add a delay to prevent race conditions
    if (authChecked && !user && !loading && !hasRedirected.current) {
      const timer = setTimeout(() => {
        // Double-check that we still don't have a user before redirecting
        if (!user && !hasRedirected.current) {
          hasRedirected.current = true;
          router.push('/?login=required&redirect=/playground');
        }
  ...
  // Authentication is now handled by useAuthGuard hook
    ...
    >
      ...
      {showProblemForm && (
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-white mx-auto mb-4"></div>
            <p className="text-white text-lg">Loading playground...</p>
          </div>
        </div>
      )}
    </div>
